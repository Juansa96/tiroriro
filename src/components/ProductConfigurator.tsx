...
import { whatsappHref, configuradorDisenoText, setConfiguradorDiseno } from "@/lib/whatsapp";
...
const ProductConfigurator = () => {
  ...
  // Diseño en curso listo para mandarlo por WhatsApp (botón «Pedir precio por
  // WhatsApp» y botón flotante). Solo cuando ya hay producto elegido.
  const whatsappDisenoText = useMemo(() => {
    if (!productType) return null;
    const summary = buildConfigSummary(productType, options);
    const precio = priceIsKnown && !isPriceOnRequest ? priceLabel(finalPrice) : undefined;
    return configuradorDisenoText(summary, precio);
  }, [productType, options, priceIsKnown, isPriceOnRequest, finalPrice]);

  useEffect(() => {
    setConfiguradorDiseno(whatsappDisenoText);
    return () => setConfiguradorDiseno(null);
  }, [whatsappDisenoText]);

  const handleWhatsappDiseno = () => {
    window.gtag?.("event", "conversion", { send_to: "AW-18316237534/ja3TCICIvewcEN617p1E" });
  };
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="w-[45%] lg:w-1/2 sticky top-20 self-start" style={{ maxHeight: 'calc(100vh - 80px)' }}>
          ...
          <div className="flex flex-col gap-3 mt-4">
            <button
              ...
              {whatsappDisenoText && (
                <a
                  href={whatsappHref(whatsappDisenoText)}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={handleWhatsappDiseno}
                  className="w-full inline-flex items-center justify-center gap-2 px-8 py-3 text-xs tracking-[0.14em] uppercase font-light border border-border hover:border-foreground/60 transition-colors"
                >
                  <WhatsappGlyph className="w-4 h-4 text-[#25D366]" />
                  Pedir precio de mi diseño por WhatsApp
                </a>
              )}
          </div>
        </div>
      ...
      <div className="md:hidden px-4 pb-40 pt-2">
        <div className="mb-3">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-background border-t border-border px-6 py-4">
        {whatsappDisenoText && (
          <a
            ...
            rel="noopener noreferrer"
            onClick={handleWhatsappDiseno}
            className="flex items-center justify-center gap-2 -mt-1 mb-3 py-2 text-[11px] tracking-[0.12em] uppercase text-foreground border border-border rounded-sm"
          >
            <WhatsappGlyph className="w-4 h-4 text-[#25D366]" />
            Pedir precio de mi diseño por WhatsApp
          </a>
...
/** Logo de WhatsApp en línea (hereda el color con currentColor). */
